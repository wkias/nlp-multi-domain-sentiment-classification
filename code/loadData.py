...
class TextIterator():
    ...
    def nextBatch(self):
        if self.config.wordemb_suffix == "mask_t":
            retX = [np.ones(shape=[self.config.batch_size, self.config.task_len[i]],
                            dtype='int64') for i in range(self.config.task)]
        else:
            retX = [np.zeros(shape=[self.config.batch_size, self.config.task_len[i]],
                             dtype='int64') for i in range(self.config.task)]
        retY = [np.zeros(shape=[self.config.batch_size])
                for i in range(self.config.task)]
        retDomain = [np.zeros(shape=[self.config.batch_size])
                     for i in range(self.config.task)]
    ...
    def getValid(self, type=0):
        ...
        retLen = min(self.config.batch_size, len(
            dataLS[0]) - indLS[0]*self.config.batch_size)
        if retLen <= 0:
            for i in range(self.config.task):
                indLS[i] = 0
            return None, None, None, None, False
        if self.config.wordemb_suffix == "mask_t":
            retX = [np.ones(shape=[retLen, self.config.task_len[i]],
                            dtype='int64') for i in range(self.config.task)]
        else:
            retX = [np.zeros(shape=[retLen, self.config.task_len[i]],
                             dtype='int64') for i in range(self.config.task)]
